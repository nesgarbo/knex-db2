...
describe('Update Statements', () =>
{
    it('supports basic update', () =>
    {
        ...
        const query = knex('test').update(updates)
            .where('x', 'y');
    ...
    it('supports update with whereExists', () =>
    {
        ...
        const query = knex('test').update(updates)
            ...
            {
                this.select('*').from('othertable')
                    .whereRaw('othertable.id = test.id');
            });
    ...
    it('supports update with whereNotBetween', () =>
    {
        const updates = { x: 10, y: 20 };
        const query = knex('test').update(updates)
            .whereNotBetween('x', [ 1, 10 ]);

        testSql(query, 'update test set x = 10, y = 20 where x not between 1 and 10');
    });
    ...
    it('supports update with a subquery in where clause', () =>
    {
        const updates = { x: 10, y: 20 };
        const query = knex('test').update(updates)
            .where('x', knex.select('id').from('othertable')
                .where('name', 'John'));
    ...
    it('supports update with returning clause', () =>
    {
        ...
        testSql(query, 'select id from FINAL TABLE(update test set x = 10, y = 20 where x = \'y\')');
    });
    ...
    it('supports update with returning clause and multiple columns', () =>
    {
        ...
        testSql(query, 'select id, x from FINAL TABLE(update test set x = 10, y = 20 where x = \'y\')');
    });
    ...
    it('supports conditional update using case/when', () =>
    {
        ...
        testSql(query, 'update test set x = case when y = 1 then 10 else 20 end where x = \'y\'');
    });
